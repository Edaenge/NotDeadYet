...
void Client::UpdateCameraPos()
{
	...
	//float angle = acos(prevDirection.GetDotProduct(camDir) / (prevDirection.GetLength() * camDir.GetLength()));

	//PlayerObject* player = this->zObjectManager->GetPlayerObject(index);
	//
	//iMesh* mesh = player->GetMesh();

	//mesh->RotateAxis(around, angle);

	//prevDirection = camDir;
}
