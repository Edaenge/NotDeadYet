...
Vector2 ObjLoader::GetTextCoordsData(string line)
{
	string su = "";
	string sv = "";
...
Vector3 ObjLoader::GetVertsNormsData(string line)
{
	string sx = "";
	string sy = "";
	string sz = "";
...
ObjData* ObjLoader::LoadObjFile(string filepath)
{
	...
	while(!file.eof())
	{
		...
		{
			Vector3 vertpos = this->GetVertPosData(line);
		...
		{
			Vector3 norm = this->GetVertsNormsData(line);
			norm.z *= -1;	// For D3DX
	...
	while(!file.eof())
	{
		...
		{
			Vector3 Kd = this->GetVertPosData(line);
			md.kd = Kd;
		}
		else if(line.size() > 0 && line.substr(0, 2) == "Ka")
		{
			Vector3 Ka = this->GetVertPosData(line);
			md.ka = Ka;
		}
		else if(line.size() > 0 && line.substr(0, 2) == "Tf")
		{
			Vector3 Tf = this->GetVertPosData(line);
			md.tf = Tf;
		}
		...
		{
			Vector3 Ks = this->GetVertPosData(line);
			md.ks = Ks;
		}
...
void ObjLoader::ReadFromBinaryFile(ObjData* returndata, ifstream& binfile)
{
	...
	for(int i = 0; i < nrOfVerts; i++)
	{
		...
		returndata->vertspos->add(Vector3(x, y, z));
	}
	...
	for(int i = 0; i < nrOfTextCoords; i++)
	{
		...
		returndata->textcoords->add(Vector2(x, y));
	}
	...
	for(int i = 0; i < nrOfNorms; i++)
	{
		...
		returndata->vertsnorms->add(Vector3(x, y, z));
	}
