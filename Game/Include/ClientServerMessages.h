...
namespace Messages
{
	...
	inline void DebugInventory(std::string msg)
	{
		if(eraseFile)
		{
			std::ofstream writeFile;
			writeFile.open ("InventoryDebugData.txt", std::ios::out | std::ios::trunc);
		...
		std::fstream writeFile;
		writeFile.open ("InventoryDebugData.txt", std::fstream::in | std::fstream::out | std::fstream::app);
